...
# Base URL of the Flask backend (container name on the Docker network)
...
# Assemble the JSON payload the backend expects
product_data = {
...
if uploaded_file is not None:
    if st.button("Predict for Batch", type="primary"):
        ...
        if response.status_code == 200:
            ...
            df = pd.DataFrame([results]) if all(not isinstance(v, (list, dict)) for v in results.values()) else pd.DataFrame(results)
